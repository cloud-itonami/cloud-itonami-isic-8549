(ns training.operation
  "OperationActor -- one training operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (EdOps-
  LLM) is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Instruction Integrity Governor (:govern) and the
  ...
  One graph run = one training operation (intake -> advise -> govern
  ...
  `:actuation/finalize-completion` ALWAYS reaches this node when the
  governor is clean -- see `training.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [training.edopsllm :as edopsllm]
...
(defn build
  ...
  opts:
    :advisor      -- a `training.edopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (edopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; EdOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (edopsllm/-advise advisor store request)]
            {:proposal p :audit [(edopsllm/trace request p)]})))
